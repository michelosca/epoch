...
CONTAINS
  ...
  SUBROUTINE collisional_ionisation
    ...
    ALLOCATE(idens(1-ng:nx+ng))
    ALLOCATE(jdens(1-ng:nx+ng))
    ALLOCATE(e_dens(1-ng:nx+ng))
    ALLOCATE(itemp(1-ng:nx+ng))
    ALLOCATE(jtemp(1-ng:nx+ng))
    ALLOCATE(e_temp(1-ng:nx+ng))
    ALLOCATE(log_lambda(1-ng:nx+ng))
    ALLOCATE(e_log_lambda(1-ng:nx+ng))
    ALLOCATE(meanx(1-ng:nx+ng))
    ALLOCATE(meany(1-ng:nx+ng))
    ALLOCATE(meanz(1-ng:nx+ng))
    ALLOCATE(part_count(1-ng:nx+ng))
    ...
    DO ispecies = 1, n_species
      ...
      DO jspecies = ispecies, n_species
        ...
        IF (ispecies == jspecies) THEN
          DO ix = 1, nx
            CALL intra_coll_fn(species_list(ispecies)%secondary_list(ix), &
                m1, q1, w1, idens(ix), log_lambda(ix), user_factor)
          END DO ! ix
        ELSE IF (species_list(ispecies)%ionise &
            .AND. species_list(jspecies)%electron) THEN
          DO ix = 1, nx
