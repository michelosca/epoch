...
#include "particle_to_grid.inc"
  ...
  SUBROUTINE calc_cou_log(data_array, current_species, direction)
    ...
    INTEGER, INTENT(IN) :: current_species
    INTEGER, INTENT(IN), OPTIONAL :: direction
    INTEGER :: ispecies, use_species, ix
    TYPE(particle), POINTER :: current
    ...
    ALLOCATE(num_dens(1-ng:nx+ng))
    ALLOCATE(mean_ke(1-ng:nx+ng))
    ALLOCATE(temperature(1-ng:nx+ng))
    ALLOCATE(num_dens_all(1-ng:nx+ng))

    ALLOCATE(debye_length(1-ng:nx+ng))
    ALLOCATE(atomic_spacing(1-ng:nx+ng))
    ALLOCATE(classical_b0(1-ng:nx+ng))
    ALLOCATE(uncertain_dx(1-ng:nx+ng))
    ...
    DO ispecies = 1, n_species
      ...
      DO ix = 1-ng, nx+ng
        IF (num_dens(ix) > 1.0e-15_num) THEN
          debye_length(ix) = debye_length(ix) &
              + temperature(ix) &
              / (num_dens(ix) * (species_charge(ispecies))**2)
        ELSE
          debye_length(ix) = 0.0_num
        END IF
      END DO
    ...
    DO ix = 1-ng, nx+ng
      ...
      IF (debye_length(ix) > 0.0_num) THEN
        debye_length(ix) = SQRT(debye_length(ix) * epsilon0 * kb)
      ELSE
        debye_length(ix) = 0.0_num
      END IF

      ! Interatomic distance, assuming each particle is a sphere
      IF (num_dens_all(ix) > 1.0e-15_num) THEN
        atomic_spacing(ix) = (3.0_num &
            / (4.0_num * pi * num_dens_all(ix)))**(1.0_num/3.0_num)
      ELSE
        atomic_spacing(ix) = 0.0_num
      END IF

      ! Classical distance of closest approach, Lee-More (20), replacing mv2
      ! with 0.5*<KE>. Initially, classical_b0 = <KE>
      IF (mean_ke(ix) > 0.0_num) THEN
        classical_b0(ix) = mean_q1q2 &
            / (8.0_num * pi * epsilon0 * mean_ke(ix))
      ELSE
        classical_b0(ix) = 0.0_num
      END IF

      ! Uncertainty principle limit, Lee-More (21)
      p_rel = SQRT((mean_ke(ix) + current_mass * c**2)**2 &
          - current_mass**2 * c**4) / c
      IF (p_rel > 0.0_num) THEN
        uncertain_dx(ix) = h_planck / (2.0_num * p_rel)
      ELSE
        uncertain_dx(ix) = 0.0_num
      END IF

      ! Coulomb logarithm, Lee-More (17)
      b_min = MAX(uncertain_dx(ix), classical_b0(ix))
      b_max = MAX(debye_length(ix), atomic_spacing(ix))
      IF (b_min > 0.0_num .AND. b_max >= b_min) THEN
        data_array(ix) = 0.5_num * LOG(1.0_num + (b_max/b_min)**2)
      ELSE
        data_array(ix) = 0.0_num
      END IF
    END DO
